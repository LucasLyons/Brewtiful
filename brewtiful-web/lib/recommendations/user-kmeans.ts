...
/**
 * Fetch candidate beers for recommendations (unrated beers with embeddings)
 * @deprecated Use getCandidateBeersFromCentroids for k-means recommendations
 * @param userId - User ID (null for anonymous users)
 * @param limit - Maximum number of candidates to fetch (default 1000)
 * @returns Array of candidate beers with embeddings
 */
export async function getCandidateBeers(
  userId: string | null,
  limit: number = 1000
): Promise<CandidateBeer[]> {
  const supabase = await createClient();
  ...
  // Fetch unrated beers with embeddings
  ...
  if (ratedBeerIds.length > 0) {
    candidatesQuery = candidatesQuery.not('beer_id', 'in', `(${ratedBeerIds.join(',')})`);
  }
